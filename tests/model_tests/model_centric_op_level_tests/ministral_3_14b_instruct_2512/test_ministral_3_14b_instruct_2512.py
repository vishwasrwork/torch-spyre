...
CPU vs Spyre comparison tests for:
  • scaled_dot_product_attention (SDPA) — Ministral-3B-Instruct-2512
  • torch.cat                           — Ministral-3-14B-Instruct-2512
  • torch.zeros                         — Ministral-3-14B-Instruct-2512

Model Architecture (Ministral-3B-Instruct-2512):
  hidden_size         : 4096
...
class TestSDPA(unittest.TestCase, metaclass=ParameterizedTestMeta):
    ...
    def test_sdpa_gqa_shape(self, q, k, v, compiled):
        """K/V already have 32 heads — shape must equal Q shape after slicing."""
        def fn(q, k, v):
            seq_q = q.shape[2]
            # Slice K/V to match Q's sequence length
            k_sliced = k[:, :, :seq_q, :]
            v_sliced = v[:, :, :seq_q, :]
            
            assert k_sliced.shape == q.shape, (
                f"K shape {tuple(k_sliced.shape)} != Q shape {tuple(q.shape)}"
            )
            assert v_sliced.shape == q.shape, (
                f"V shape {tuple(v_sliced.shape)} != Q shape {tuple(q.shape)}"
            )
            
            # Run the actual SDPA so the function has a tensor output.
            seq_kv = k_sliced.shape[2]
            mask = torch.full(
                (1, 1, seq_q, seq_kv),
                float("-inf"),
                dtype=q.dtype,
            ...
            for i in range(seq_q):
                mask[:, :, i, : i + 1] = 0.0
            return _sdpa_fn_no_expand(q, k_sliced, v_sliced, attn_mask=mask)
    ...
    def test_sdpa_gradient_flow(self, q, k, v, compiled):
        ...
        compare_with_cpu(fn, q, k, v, compiled=False, enable_grad=True)   # gradient test: eager only
                ...
                "add_bias_1x64x1_1x1x14_eager":    
                    (make_strided_tensor((1, 64, 1), (64, 1, 1), torch.float16), 
                     make_strided_tensor((1, 1, 14), (14, 14, 1), torch.float16), 
                      lambda a, b: torch.matmul(a, b) + torch.ones(1, 64, 14, dtype=torch.float16, device=a.device), 
                      False),
