...
# ─────────────────────────────────────────────────────────────────────────────
def _run_on_device(fn, args, device, compiled, needs_device=False, enable_grad=False):
    """
    Run ``fn(*args)`` on the specified device, optionally compiling it.
    ...
    enable_grad  : if True, allows gradient computation (default: False).
    """
    if compiled:
...
# ─────────────────────────────────────────────────────────────────────────────
# Spyre layout helpers removed ──────────────────────
# All layout-related functions (_get_spyre_layout, _is_shape_changing_op,
# _assert_spyre_layout_equal, _assert_spyre_layout_equal_recursive,
# validate_cpu_spyre_layout) have been removed. This version only performs
# numerical comparisons without layout validation.
...
def compare_sdpa(
    ...
    *cpu_args,
    ...
    """Run ``fn(*cpu_args)`` eagerly on CPU and on Spyre; assert numerical closeness.

    Comparison strategy
    -------------------
    This no-dimmap version performs only numerical comparison between CPU and
    Spyre results. Layout validation has been removed.

    Steps
    -----
    1. Run ``fn(*cpu_args)`` on CPU → ``cpu_out``.
    2. Move tensor args to ``DEVICE``; run ``fn(*spyre_args)`` → ``spyre_out``.
    3. Assert numerical closeness between ``spyre_out`` and ``cpu_out``.
...
def compare_with_cpu(
    ...
    *args,
    ...
    """Run ``fn(*args)`` on both CPU and Spyre and assert numerical closeness.
    ...
    *args        : positional arguments.  Tensor args are moved to the target
    ...
    atol / rtol  : numerical tolerances for comparison. If not provided,
                   defaults to COMPILED_ATOL/RTOL or EAGER_ATOL/RTOL based
                   on the compiled flag.
    enable_grad  : if True, allows gradient computation (default: False).
    """
    ...
    cpu_out = _run_on_device(
        fn, args, device="cpu", compiled=compiled, needs_device=needs_device, enable_grad=enable_grad
    )

    # Step 2 — Spyre execution.
    spyre_out = _run_on_device(
        fn, args, device=DEVICE, compiled=compiled, needs_device=needs_device, enable_grad=enable_grad
    )
